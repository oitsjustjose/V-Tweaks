...
public class MobTweakConfig
{
        ...
        public static void init(ForgeConfigSpec.Builder COMMON_BUILDER)
        {
                ...
                CHALLENGER_MOBS_LOOT = COMMON_BUILDER.comment(
                                "A string list of form <modid:item> of possible loot items a Challenger Mob could drop")
                                .defineList("challengerMobsLoot",
                                                Lists.newArrayList("minecraft:gold_ingot", "minecraft:gold_nugget*15",
                                                                "minecraft:diamond", "minecraft:emerald",
                                                                "minecraft:ghast_tear", "minecraft:ender_pearl",
                                                ...
                                                (itemRaw) -> {
                                                        if (itemRaw instanceof String)
                                                        {
                                                                ...
                                                                Item item = ForgeRegistries.ITEMS
                                                                                .getValue(new ResourceLocation(parts[0],
                                                                                                parts[1]));

                                                                if (item != null)
                                                                {
                                                                        challengerMobDrops.add(new ItemStack(item,
                                                                                        parts.length == 3 ? Integer
                                                                                                        .parseInt(parts[2])
                                                                                                        : 1));
                                                                        return true;
                                                                }
                ...
                CHALLENGER_MOBS_BLACKLIST = COMMON_BUILDER.comment(
                ...
                PEACEFUL_SURFACE_MIN_Y = COMMON_BUILDER
                                .comment("The lowest Y-level which mobs will be prevented from spawning")
                                .defineInRange("peacefulSurfaceMinY", 60, 1, 255);
                PEACEFUL_SURFACE_BLACKLIST = COMMON_BUILDER.comment(
