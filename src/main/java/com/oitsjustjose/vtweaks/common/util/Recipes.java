    ...
    public void registerBookRecipes(AnvilUpdateEvent event) {
        ...
        if (EnchantmentConfig.ENABLE_IMPERISHABLE.get()) {
            ...
            if (event.getLeft().getItem() == Items.ENCHANTED_BOOK
                ...
                if (Utils.bookHasEnchantment(event.getLeft(), new EnchantmentData(Enchantments.UNBREAKING, 3)) && Utils
                        .bookHasEnchantment(event.getRight(), new EnchantmentData(Enchantments.UNBREAKING, 3))) {
